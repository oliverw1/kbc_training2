...
tv_sessions = (
    ...
    .withColumn(
        "date",
        fun.from_unixtime(
            fun.unix_timestamp(tv_sessions.sessionRecordStartTime), "yyyy-MM-dd"
        ),
    )
)
...
tv_sessions = (
    tv_sessions.withColumn(
        "kijkmaand", substring_f(startpos=6, lengte=2)(tv_sessions.date)
    )
    .withColumn(
        "hoofdgenre", substring_f(startpos=1, lengte=3)(tv_sessions.programGenreLevel)
    )
    .withColumn(
        "season", substring_f(startpos=16, lengte=2)(tv_sessions.programSeasonEpisode)
    )
    .withColumn(
        "episode", substring_f(startpos=19, lengte=4)(tv_sessions.programSeasonEpisode)
    )
)

tv_sessions = tv_sessions.filter(
    tv_sessions.kijkmaand.isin("01", "02", "03", "04", "05", "06")
).filter(
    (tv_sessions.hoofdgenre == "13.") & (tv_sessions.programGenreLevel != "13.6")
)  # geen soaps
...
maxepisodes = tv_sessions.groupBy("customerNumber", "programSeriesName", "season").agg(
    fun.max(tv_sessions.episode).alias("max_episode")
)
...
samen = firstepisodes.join(
    maxdate, ["customerNumber", "programSeriesName", "season"], "left outer"
).drop("season")

samenfiltered = samen.withColumn(
    "days_spread",
    fun.datediff(samen.last_sessionRecordStartTime, samen.first_sessionRecordStartTime),
)
